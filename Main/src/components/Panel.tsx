...
import { SettingsPanel } from "./model-settings/SettingsPanel.js";
...
export function computePanelSize(
	...
	return {
		width: Math.max(
			PANEL_MIN_WIDTH,
			Math.min(baseSize.width, window.innerWidth - gap * 2),
		),
		height: Math.max(
			PANEL_MIN_HEIGHT,
			Math.min(baseSize.height, window.innerHeight - gap * 2),
		),
	};
}
...
export function Panel({
	...
	const [storedPosition, setStoredPosition] = useState<PanelPosition | null>(
		null,
	);
	const [storedSize, setStoredSize] = useState<PanelSize | null>(null);
	const [storedAnchorRect, setStoredAnchorRect] =
		useState<ToggleButtonRect | null>(null);

	useEffect(() => {
		chrome.storage.local
			.get([
				"bili-agent-panel-position",
				"bili-agent-panel-size",
				"bili-agent-panel-anchor-rect",
			])
			.then((result) => {
				const pos = result["bili-agent-panel-position"] as
					| PanelPosition
					| undefined;
				const sz = result["bili-agent-panel-size"] as PanelSize | undefined;
				const anchor = result["bili-agent-panel-anchor-rect"] as
					| ToggleButtonRect
					| undefined;
	...
	const initialSize = useMemo(
		() => computePanelSize(storedSize ?? defaultSize),
	...
	const hasButtonMovedSignificantly = useMemo(() => {
		...
		return (
			Math.hypot(
				currentCenterX - storedCenterX,
				currentCenterY - storedCenterY,
			) > 96
		);
	...
	const initialPosition =
		storedPosition && !hasButtonMovedSignificantly
			? storedPosition
			: defaultPosition;
	...
	const handleDragEnd = useCallback(
		(pos: { x: number; y: number }) => {
			...
			if (anchorRect) {
				chrome.storage.local.set({
					"bili-agent-panel-anchor-rect": anchorRect,
				});
			}
		},
	...
	const panelTransformOrigin = useMemo(() => {
		...
		const originX = clamp(
			((buttonCenterX - effectivePosition.x) / size.width) * 100,
			0,
			100,
		);
		const originY = clamp(
			((buttonCenterY - effectivePosition.y) / size.height) * 100,
			0,
			100,
		);
	...
	const [settingsSnapshot, setSettingsSnapshot] =
		useState<BiliAgentSettings | null>(null);

	/**
	 * 设置专用独立 Port（P5-1.8 关键架构修复）。
	 *
	 * 设计依据 SA-12 经单 Port：Panel.tsx 第 487-506 行设置分支与 ChatProvider 分支
	 * 互斥渲染，ChatProvider 内的 Port 不可达，因此 Panel 组件自身在 isSettingsOpen
	 * 且设置需要测试连接时调 chrome.runtime.connect 建立独立 Port（设置专用，不走聊天流），
	 * 作为 port prop 传入 SettingsPanel，再下传给 TestConnectionButton。
	 *
	 * Port name 复用 'bili-agent-chat'：SW 端 setupPortListener 监听该 name，
	 * test_connection 消息已在 switch 中路由，无需 SW 端改动。
	 */
	const [settingsPort, setSettingsPort] = useState<chrome.runtime.Port | null>(
		null,
	);
	...
	/**
	 * 设置页打开时建立独立 Port（设置专用，不走聊天流）。
	 *
	 * 设计依据 SA-12 经单 Port：设置分支与 ChatProvider 分支互斥渲染，
	 * ChatProvider 的 Port 不可达。Panel 组件自身在此建立独立 Port 传入 SettingsPanel，
	 * 再下传给 TestConnectionButton 执行连接测试。
	 *
	 * Port name 复用 'bili-agent-chat'：SW 端 setupPortListener 监听该 name，
	 * test_connection 消息已在 switch 中路由，无需 SW 端改动。
	 *
	 * 关闭设置时 disconnect 释放资源，避免 Port 泄漏。
	 */
	useEffect(() => {
		if (!isSettingsOpen) return;
		...
		try {
			...
			// chrome.runtime.connect 失败时 port 保持 null，渲染层会显示错误提示
		...
		>
			...
			>
				...
				>
					...
					{isSettingsOpen && (
						<span className="bili-agent-panel__badge">Settings</span>
					)}
					...
					<HistoryDropdown
						...
						onLoad={async (id) => {
							...
							window.dispatchEvent(
								new CustomEvent("biliagent:history-load", {
			...
			{isSettingsOpen ? (
				settingsSnapshot && settingsPort ? (
					...
					<div
						className="bili-agent-panel__chat"
						style={{ padding: 16, color: "#999", fontSize: 13 }}
					>
						加载设置中...
					</div>
				)
			...
			{resizeHandles.map((handle) => {
				const isHorizontal =
					handle.direction.includes("e") || handle.direction.includes("w");
				const isVertical =
					handle.direction.includes("n") || handle.direction.includes("s");
			...
			<div
				className={`bili-agent-panel__footer${isSettingsOpen ? " bili-agent-panel__footer--visible" : ""}`}
			>
				BiliGo v0.0.1
			</div>
		</aside>
